...
-- 2017-01-01,PLandeis,Created File
--**************************************************************************--
Begin Try
	Use Master;
	If Exists(Select Name From SysDatabases Where Name = 'Assignment06DB_PLandeis')
	 Begin 
	  Alter Database [Assignment06DB_PLandeis] set Single_user With Rollback Immediate;
	  Drop Database Assignment06DB_PLandeis;
	 End
	Create Database Assignment06DB_PLandeis;
End Try
Begin Catch
	Print Error_Number();
End Catch
go
Use Assignment06DB_PLandeis;
...
AS
  SELECT TOP 100 PERCENT CategoryName, ProductName, UnitPrice
...
AS
  SELECT TOP 100 PERCENT ProductName, InventoryDate, [Count]
...
AS
  SELECT TOP 100 PERCENT CategoryName, ProductName, InventoryDate, [Count]
  FROM Categories INNER JOIN Products
	ON Categories.CategoryID = Products.CategoryID
...
AS
  SELECT TOP 100 PERCENT CategoryName, ProductName, InventoryDate, [Count], EmployeeFirstName + ' ' + EmployeeLastName AS EmployeeName
  FROM Categories INNER JOIN Products
	ON Categories.CategoryID = Products.CategoryID
  INNER JOIN Inventories
	ON Products.ProductID = Inventories.ProductID
...
AS
  SELECT TOP 100 PERCENT M.EmployeeFirstName + ' ' + M.EmployeeLastName AS Manager, E.EmployeeFirstName + ' ' + E.EmployeeLastName AS Employee
